...
> {
  ...
  async findWithRelations(options?: FindManyOptions<People>) {
    return await this.repository.find({
      relations: ['homeworld', 'species', 'starships', 'vehicles', 'files'],
      ...options,
    });
  }

  async addRelations(ent: People, rel: Partial<RIDPeople>) {
    if (rel.films) {
